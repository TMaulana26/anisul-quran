...
| **B-25** | **Audio Qari Non-Mishary Melompat ke Detik Selesai Surah Sebelumnya (00:16 atau 00:50) Saat Pindah Surah** | Ketika surah selesai, `currentAyahNumber` tersisa pada ayat terakhir (misal Ayat 7 di Al-Fatihah, Ayat 4 di Al-Ikhlas). Untuk qari non-default (selain Mishary ID 7), `handleSelectReciter` membaca `currentAyahNumber` lama tanpa memeriksa apakah surah berganti, sehingga surah baru (Al-Baqarah) langsung memutar dari Ayat 7 (detik 50) atau Ayat 4 (detik 16). | Menambahkan parameter `targetAyah`, memastikan evaluasi `isSameSurah`, mereset `currentAyahNumber` ke 1 saat event audio `ended`, menyematkan query `?reciter=` pada navigasi antar-surah, serta mengirimkan `startAyah = 1` secara eksplisit saat inisialisasi surah baru. | `[x]` |
